...
class MenuController extends Controller
{
    ...
    public function lista()
    {
        ...
        #----------------------------------------------------------------
        return view('admin.configuracion.menus.lista', get_defined_vars());
    }
    public function listar()
    {
        ...
        ->addColumn('padre', function ($data) {
            ...
            $padre = Menus::find($data->padre_id);
    ...
    public function formularioMenu(Request $request)
    {
        #variables que renderizan el menu y sus modulos usarlos en todos los controladores
        $modulos_padre = Menus::where('visible',true)->where('estado',true)->where('padre_id',0)->get();
        $modulos_hijos = Menus::where('visible',true)->where('estado',true)->where('padre_id','!=',0)->get();
        #----------------------------------------------------------------
        $modulo = Menus::find($request->id);
        $tipo = $request->tipo;
        return view('admin.configuracion.menus.menu-formulario', get_defined_vars());
    }
    public function guardar(Request $request)
    {
        try {
            $modulo = Menus::firstOrNew(['id' => $request->id]);
