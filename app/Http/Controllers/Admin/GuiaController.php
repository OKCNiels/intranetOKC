...
use App\Models\Almacen\DespachosExternosView;
use App\Models\Almacen\OrdenDespachos;
...
class GuiaController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.control.guias.index', get_defined_vars());
    }
    ...
    public function listar(Request $request)
    {
        ...
        ->addColumn('adj_guia', function ($data) {
            $ruta = url('admin/documentos/guias_remision/gr_adjuntos');
        ...
        ->addColumn('adj_guia_sellada', function ($data) {
            $ruta = url('admin/documentos/guias_remision/gr_selladas');
        ...
        ->addColumn('empresa_codigo', function ($data) {
            $empresa = Empresas::find($data->empresa_id);
        ...
        // try {
            ...
            $data = GuiaDespacho::firstOrNew(['control_almacen_id' => $request->id_control]);
                ...
                if ($request->hasFile('adjunto_guia')) {
                    ...
                    $file_path = public_path().'/admin/documentos/guias_remision/gr_adjuntos/'.$file_name;
                    $file->move(public_path().'/admin/documentos/guias_remision/gr_adjuntos/', $file_name);
                ...
                if ($request->hasFile('adjunto_guia_sellada')) {
                    ...
                    $file_name = uniqid().'-'.time().'.'.$file->getClientOriginalExtension();
                    $file_path = public_path().'/admin/documentos/guias_remision/gr_selladas/'.$file_name;
                    $file->move(public_path().'/admin/documentos/guias_remision/gr_selladas/', $file_name);
            ...
            if ($despachos_externos) {
                ...
                $actualOD = OrdenDespachos::find($despachos_externos->id_od);
    ...
    public function buscarOD(Request $request) {
        $data_json = array();
        $data = OrdenDespachos::where('id_requerimiento',$request->requerimiento_id)
        ->first();
        if($data) {
            $despachos_externos = $this->despachosExternos();
            $despachos_externos = $despachos_externos->where('alm_req.id_requerimiento',$request->requerimiento_id);
    ...
    public function reporteFiltros(Request $request) {
        ...
        foreach ($array as $key => $item) {
            ...
            if($item->empresa_id!==null){
                $empresa = Empresas::find($item->empresa_id);
